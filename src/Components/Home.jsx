    ...
    const fetchData = async () => {
      try {
        ...
        if (data.articles && data.articles.length > 0) {
          const firstArticle = data.articles[0]; // ✅ Corrected variable name
          ...
          setTrendingNews(data.articles.slice(1, 6)); // ✅ Set Trending News
        }
      } catch (error) {
...
>
  ...
  <div className="relative z-10 text-center max-w-2xl p-6">
    ...
    <p className="text-lg mt-4 p-4 bg-black/70 text-gray-300 rounded-lg shadow-md">
      {description || "Stay updated with the latest news worldwide."}
    </p>
